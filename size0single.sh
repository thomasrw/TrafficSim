...
while [ $i -lt 1 ]
do
/work/thoma525/helloTraci_log.py singleCAV$PERCENT"_${j}""_${SIZE}" CAV$PERCENT"_${j}" $SIZE --nogui &
j=$((j + 1))
i=$((i + 1))
done
wait
...
while [ $i -lt 1 ]
do
python3 /work/thoma525/eval_3.py singleCAV$PERCENT $j singleCAV$PERCENT"_size_${SIZE}" $SIZE &
j=$((j + 1))
i=$((i + 1))
done
wait
