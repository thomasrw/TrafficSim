...
trip = "none"
def speedv(baseline=baseline, input=net_input, trip=trip):
    '''
    #todo implement write to log file
    check baseline vehicles
    add up absolute speed changes per timestep
    divide by veh count
    :param baseline: CAV000 tripinfo
    :param input: CAVxxx netsat
    :param trip: CAVxxx tripinfo
    :return:
    '''
    basetree = ElementTree.parse(baseline)
    baseroot = basetree.getroot()
    veh = {}
    veh2 = {}
    tree = ElementTree.parse(trip)
    root = tree.getroot()
    speed_iter = ElementTree.iterparse(input)
    abs_speed_change = 0
    count = 0

    #build list of veh to assess
    for att in baseroot:
        veh[att.attrib['id']] = 0


    #todo iterate over netstate data
    # get core name for comparison
    # if core is on list, take abs difference of speed from last speed
    ...
    for event, att in speed_iter:
        if att.tag == 'vehicle':
            if att.attrib['id'] in veh2.keys():
                #print('key match')
                #todo adjust below lines to get delta on 'speed' and then set veh2 key value to current speed
                abs_speed_change += abs(float(att.attrib['speed']) - float(veh2[att.attrib['id']]))
                veh2[att.attrib['id']] = att.attrib['speed']
                print(abs_speed_change)
            else:
                print('no key match')
        att.clear()

    return abs_speed_change / count
...
Input = mypath + prefix + '_'+ str(run) + '_tripinfo'
Log = mypath + prefix +'_metrics.csv'
Platoon = mypath + prefix + '_' + str(run) + '_platoon_status.xml'
...
myPath = '/work/thoma525/'
log = myPath + 'single_speedv.csv'
prefix = sys.argv[1] #singleCAV$PERCENT
run = sys.argv[2] #$j
size = sys.argv[3] #$SIZE

baseline = myPath + "CAV000_" + run + "_tripinfo"
netstate = myPath + prefix + "_" + run + "_" + size + "_netstate"
trip = myPath + prefix + "_" + run + "_" + size + "_tripinfo"


value = speedv(baseline, netstate, trip)

try:
    if not path.exists(log):
        f = open(log, "w")
        f.write('Input, avg abs speed change\n')
    else:
        f = open(log, "a")

    f.write(netstate +','+ str(value) +'\n')
    f.close()
    print('wrote to single_speedv.csv ok')
except:
